...
# This server might not run properly on Windows
...
class Pigeon_Server:
    ...
    def start(self):
        ...
        # close all connections when server terminates
    ...
    def connect_loop(self):
        while self.ALIVE:
            try:
                reads, writes, errs = select.select(self.connections, [], [])
                for conn in reads:
                    self.handle_read(conn)
            except select.error:
                print "Interrupted system call 'select'. This is normal."
    ...
    def handle_message(self, conn, data):
        ...
        if mesg == C.REGISTER:
            ...
            print "Sending userlist to " + name + " at " + client_addr
    ...
    def handle_server(self, conn):
        ...
        print "Received initial connection from " + addr[0]
        new_conn.setblocking(0)
        self.connections.append(new_conn)

    # TODO: consider notifying clients of server's termination
